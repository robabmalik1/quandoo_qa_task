...
class TestApi():
    ...
    def test_05_verify_get_response_with_no_user_id(self, test_setUp):
        api_status_code = Methods.get_single_user_api(self, id='')
        assert api_status_code == None, 'API Call didnt proceed with empty single user id'
    ...
    def test_07_register_user_with_invalid_user(self, test_setUp):
        ...
        api_status_code = Methods.get_registeration_status_code(self, single_user_email, password=self.valid_password)
        assert api_status_code == 400, 'User registered with invalid credentials, test case failed'
    ...
    def test_08_register_user_with_valid_email_but_no_password(self, test_setUp):
        single_user_email = Methods.get_single_user_email(self, self.valid_user_id)
        api_status_code = Methods.get_registeration_status_code(self, user_email=single_user_email, password='')
        assert api_status_code == 400, 'User registered with only valid user email but no password, test case failed'
    ...
    def test_09_register_user_with_no_email_but_valid_password(self, test_setUp):
        api_status_code = Methods.get_registeration_status_code(self, user_email='', password=self.valid_password)
        assert api_status_code == 400, 'User registered with no email only password, test case failed'
    ...
    def test_10_register_user_with_invalid_email_but_no_password(self, test_setUp):
        api_status_code = Methods.get_registeration_status_code(self, user_email=self.invalid_user_email, password='')
        assert api_status_code == 400, 'User registered with only user email but no password, test case failed'
    ...
    def test_11_register_user_with_no_email_and_no_password(self, test_setUp):
        api_status_code = Methods.get_registeration_status_code(self, user_email='', password='')
        assert api_status_code == 400, 'User registered with no user email and no password, test case failed'
